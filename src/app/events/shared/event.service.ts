...
export class EventService {
  ...
  getEvent(id: number): Observable<IEvent> {
    ...
    const url: string = '/api/events/' + id;
    return this.http
      .get<IEvent>(url)
  ...
  //     var mathingSessions = event.sessions.filter(
  //       (session) => session.name.toLocaleLowerCase().indexOf(term) > -1
  //     );
  //     mathingSessions = mathingSessions.map((session: any) => {
  //       session.eventId = event.id;
  //       return session;
  //     });
  //     results = results.concat(mathingSessions);
  //   });
  ...
  searchSessions(searchTerm: string): Observable<ISession[]> {
    const url: string = '/api/sessions/search?search=' + searchTerm;
    return this.http
      .get<ISession[]>(url)
