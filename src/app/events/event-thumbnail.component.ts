...
@Component({
  ...
  template: `<div
    ...
    class="well hoverwall thumbnail"
  >
    <h2>{{ event?.name | uppercase }}</h2>
    <div>Date:{{ event?.date | date: 'd/M/y' }}</div>
    <div
      [ngStyle]="getStartTimeStyle()"
      [ngClass]="getStartTimeClassStr()"
      [ngSwitch]="event?.time"
    >
      Time:{{ event?.time }}
      <span *ngSwitchCase="'8:00 am'" id="earlier">
        <strong>(Early Start)</strong></span
      >
      <span *ngSwitchCase="'10:00 am'" id="lately">
        <strong>(Late Start)</strong></span
      >
      <span *ngSwitchDefault id="normaly">
        <strong>(Normal Start)</strong></span
      >
    </div>
